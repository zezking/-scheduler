...
test("getAppointmentsForDay returns an array containing the correct appointment objects", () => {
  const [first, second, third] = getAppointmentsForDay(state, "Tuesday");
  expect(first).toEqual(state.appointments["4"]);
  expect(second).toEqual(state.appointments["5"]);
  expect(third).toEqual(state.appointments["7"]);
});
...
//get the interview details, including the interviewers and student name
